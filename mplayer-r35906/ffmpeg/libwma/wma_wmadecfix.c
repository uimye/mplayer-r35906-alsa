...
void ff_imdct_calc_fixed(MDCTContextFix *s, int *outputfix,
                   const int *input, FFTSample_fix *tmp);
...
int ff_mdct_init_fixed(MDCTContextFix *s, int nbits, int inverse)
{
    ...
    s->n = n;
    n4 = n >> 2;
    s->tcosfix = av_malloc(n4 * sizeof(FFTSample_fix));
    if (!s->tcosfix)
        goto fail;
    s->tsinfix = av_malloc(n4 * sizeof(FFTSample_fix));
    if (!s->tsinfix)
        goto fail;
...
int wma_decode_close(AVCodecContext *avctx)
{  
